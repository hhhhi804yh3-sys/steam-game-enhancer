...
export const Route = createFileRoute("/switch-admin/telemetry")({
  head: () => ({ meta: [{ title: "Live Telemetry & Users â€” SteamSwitch" }] }),
...
function getStoredTelemetry(): Row[] {
  try {
    const raw = localStorage.getItem("SteamSwitch_cached_telemetry");
...
function saveStoredTelemetry(rows: Row[]) {
  try {
    localStorage.setItem("SteamSwitch_cached_telemetry", JSON.stringify(rows));
  } catch {}
}
...
function TelemetryPage() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/public/switch/telemetry").then(r => r.json()).catch(() => null);
      ...
      <header className="border-b border-slate-800/80 bg-slate-950/60 backdrop-blur-xl sticky top-0 z-40">
        <div className="mx-auto max-w-7xl px-5 py-3.5 flex items-center justify-between">
          <Link to="/switch-admin/dashboard" className="flex items-center gap-3">
            <LogoMark size={28} />
            <span className="font-bold text-lg tracking-tight">SteamSwitch<span className="text-cyan-400">Tools</span> <span className="text-xs text-slate-500 font-mono">/ Live</span></span>
          </Link>
        ...
        <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-4 mb-6 flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="relative w-full md:w-96">
            ...
            <input 
              ...
              placeholder="Search by IP, OS, hostname, user, token, countryâ€¦" 
              value={q} 
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="text-xs text-slate-400 truncate mt-0.5 font-mono">
                        Token: <span className="text-slate-300">{r.session_token.slice(0, 16)}...</span> â€¢ IP: <span className="text-slate-300">{r.ip || "â€”"}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="border-t border-slate-800/80 bg-slate-950/70 p-5 space-y-5 animate-fade-in">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs font-mono">
                      <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3 space-y-1.5">
                        ...
                        <div><span className="text-slate-500">CPU:</span> {r.cpu || "â€”"}</div>
                        <div><span className="text-slate-500">GPU:</span> {r.gpu || "â€”"}</div>
                        <div><span className="text-slate-500">RAM:</span> {r.ram_mb ? `${r.ram_mb} MB` : "â€”"}</div>
                        <div><span className="text-slate-500">Resolution:</span> {r.screen || "â€”"}</div>
                      </div>
                      ...
                      <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3 space-y-1.5">
                        ...
                        <div><span className="text-slate-500">IP Address:</span> {r.ip || "â€”"}</div>
                        <div><span className="text-slate-500">Country:</span> {r.country || "â€”"}</div>
                        <div><span className="text-slate-500">City:</span> {r.city || "â€”"}</div>
                        <div><span className="text-slate-500">Timezone:</span> {r.timezone || "â€”"} ({r.locale || "â€”"})</div>
                      </div>
                      ...
                      <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3 space-y-1.5">
                        ...
                        <div><span className="text-slate-500">Steam ID:</span> {r.steam_id || "â€”"}</div>
                      </div>
                    </div>
